...
function parseEmailItems(emailText){
    ...
    var items = [];
    var wRe = /Width:\s*([\d.]+)/i;
    var hRe = /Height:\s*([\d.]+)/i;
    var fRe = /Image file upload:\s*([^\r\n]+)/i;

    for (var j = 0; j < markers.length; j++){
        var itemStart = markers[j].itemStart;
        var itemEnd = (j + 1 < markers.length) ? markers[j + 1].itemStart : sectionEnd;
        var block = text.substring(itemStart, itemEnd);
        var wm = wRe.exec(block);
        var hm = hRe.exec(block);
        var fm = fRe.exec(block);
        if (!wm || !hm || !fm) continue;

        var widthIn = parseFloat(wm[1]);
        var heightIn = parseFloat(hm[1]);
        ...
        items.push({
            ...
            height: heightIn,
            file: fileName,
            productLabel: markers[j].productLabel,
            printType: detectPrintType(markers[j].productLabel),
...
// selection bounds union (visibleBounds) -> [L,T,R,B]
function getSelectionBounds(doc){
    var sel = doc.selection;
    if (!sel || sel.length === 0) return null;

    var b = sel[0].visibleBounds.slice(0);
    for (var i=1;i<sel.length;i++){
        var bb = sel[i].visibleBounds;
...
function scaleSelection(doc, sx, sy){
    // required: select all before resize
...
function fitArtboardToArtwork(doc, paddingPt){
    app.executeMenuCommand("selectall");
    var b = getSelectionBounds(doc);
    if (!b) return false;
...
function buildMeasuredVisualState(resizeMode, widthPct, heightPct){
    ...
    return { severity: severity, direction: direction, rowClass: rowClass, comparePct: primaryPct };
}
...
function buildReportStats(reportRows){
    var stats = { exported: 0, check: 0, notOk: 0, errors: 0 };
...
function buildReportHtml(reportMeta, reportRows){
    ...
    html.push("var sortState={key:'',asc:true};function sortReport(key){var tbody=document.getElementById('report-body');var rows=[];for(var i=0;i<tbody.rows.length;i++)rows.push(tbody.rows[i]);if(sortState.key===key)sortState.asc=!sortState.asc;else{sortState.key=key;sortState.asc=true;}rows.sort(function(a,b){var av='',bv='';if(key==='status'){av=parseFloat(a.getAttribute('data-status-sort')||'999');bv=parseFloat(b.getAttribute('data-status-sort')||'999');}else if(key==='print'){av=(a.getAttribute('data-print-sort')||'').toLowerCase();bv=(b.getAttribute('data-print-sort')||'').toLowerCase();}else if(key==='delta'){av=parseFloat(a.getAttribute('data-delta-sort')||'0');bv=parseFloat(b.getAttribute('data-delta-sort')||'0');}else if(key==='qty'){av=parseFloat(a.getAttribute('data-qty-sort')||'0');bv=parseFloat(b.getAttribute('data-qty-sort')||'0');}else if(key==='price'){av=parseFloat(a.getAttribute('data-price-sort')||'0');bv=parseFloat(b.getAttribute('data-price-sort')||'0');}else if(key==='file'){av=(a.getAttribute('data-file-sort')||'').toLowerCase();bv=(b.getAttribute('data-file-sort')||'').toLowerCase();}else return 0;if(av<bv)return sortState.asc?-1:1;if(av>bv)return sortState.asc?1:-1;var ai=parseInt(a.getAttribute('data-row-index')||'0',10);var bi=parseInt(b.getAttribute('data-row-index')||'0',10);return ai-bi;});for(var j=0;j<rows.length;j++){tbody.appendChild(rows[j]);rows[j].cells[0].innerHTML=String(j+1);}var headers=document.getElementsByTagName('th');for(var h=0;h<headers.length;h++){var k=headers[h].getAttribute('data-key');var ind=headers[h].getElementsByClassName('sort-ind');if(ind&&ind[0])ind[0].innerHTML=(k===sortState.key?(sortState.asc?'&uarr;':'&darr;'):'');}}function toggleReviewed(cb){var tr=cb;while(tr&&tr.tagName!=='TR')tr=tr.parentNode;if(!tr)return;if(cb.checked)tr.className+=' review-muted';else tr.className=tr.className.replace(/\\breview-muted\\b/g,'').replace(/\\s+/g,' ').replace(/^\\s+|\\s+$/g,'');}");
    ...
    html.push("<div class='legend'><span class='lg-rw'>Bigger 5-10%</span><span class='lg-rn'>Bigger 10%+</span><span class='lg-sw'>Smaller 5-10%</span><span class='lg-sn'>Smaller 10%+</span><span class='lg-mw'>Mixed Stretch</span><span class='lg-er'>Error / Missing</span></div>");
...
function buildProofHtml(reportMeta, reportRows){
    ...
    for (var i = 0; i < reportRows.length; i++){
        var row = reportRows[i];
        if (!row.thumbPath || row.status === "BAD_WIDTH_HEIGHT" || row.status === "MISSING_FILE" || row.status === "OPEN_FAIL" || row.status === "ACTION_FAIL" || row.status === "PROCESS_ERROR" || row.status === "LOCKED_CONTENT") continue;
...
function buildPricingAuditHtml(reportMeta, reportRows){
    ...
    html.push("function updateSummary(){var rows=document.getElementById('audit-body').rows;var ordered=0,adjusted=0,calc=0;for(var i=0;i<rows.length;i++){var price=parseNum(rows[i].getAttribute('data-price'));var adj=parseNum(rows[i].getAttribute('data-adjusted-price'));if(!isNaN(price))ordered+=price;if(!isNaN(adj)){adjusted+=adj;calc++;}}var cur=document.body.getAttribute('data-currency')||'$';var taxRate=parseNum(document.body.getAttribute('data-tax-rate'));if(isNaN(taxRate))taxRate=13;ordered=roundMoney(ordered);adjusted=roundMoney(adjusted);var diff=roundMoney(adjusted-ordered);var diffTax=roundMoney(diff*taxRate/100);var due=roundMoney(diff+diffTax);document.getElementById('sum-price-diff').innerHTML=formatMoney(diff,cur);document.getElementById('sum-tax').innerHTML=formatMoney(diffTax,cur);document.getElementById('sum-due').innerHTML=formatMoney(due,cur);document.getElementById('sum-meta').innerHTML='Old subtotal: '+formatMoney(ordered,cur)+' | New subtotal: '+formatMoney(adjusted,cur)+' | Recalculated rows: '+calc+' / '+rows.length;}");
    ...
    html.push("</script></head><body data-currency='" + escHtml(reportMeta.currency || "$") + "' data-tax-rate='" + escHtml(isNaN(reportMeta.taxRate) ? 13 : reportMeta.taxRate) + "' onload='initAudit()'>");
...
function buildLogSummary(reportMeta, reportRows){
    var stats = buildReportStats(reportRows);
    return "\r\nSummary | Exported: " + stats.exported + " | Check: " + stats.check + " | Not OK: " + stats.notOk + " | Errors: " + stats.errors + "\r\n";
}
...
function writeTextFile(fileObj, text){
    ...
    try {
        ...
        if (!fileObj.open("w")) return { ok: false, error: "open failed: " + fileObj.error };
        opened = true;
        if (!fileObj.write(text)) {
            try { fileObj.close(); } catch (eWriteClose) {}
            opened = false;
            return { ok: false, error: "write failed: " + fileObj.error };
        }
        if (!fileObj.close()) { opened = false; return { ok: false, error: "close failed: " + fileObj.error }; }
        opened = false;
...
function appendTextFile(fileObj, text){
    ...
    try {
        fileObj.encoding = "UTF-8";
        if (!fileObj.open("a")) return { ok: false, error: "open failed: " + fileObj.error };
        opened = true;
        if (!fileObj.write(text)) {
            try { fileObj.close(); } catch (eWriteClose) {}
            opened = false;
        ...
        if (!folderPath.text){
            ...
            if (!inputFolder.exists){
                ...
                if (items.length === 0){
                    ...
                    var emailFinancials = parseEmailFinancials(emailText, items);
                    var reportMeta = {
                        appName: "Illustrator",
                    ...
                    var logState = initManagedLog(new File(logPath), buildLogHeader(reportMeta));
                    logPath = logState.path || logPath;
                    logWriteWarning = logState.warning || "";
                    logWriteError = logState.error || "";
                    ...
                    function recordRow(row){
                        reportRows.push(row);
                        appendManagedLog(logState, formatLogRow(reportRows.length, row));
                        logPath = logState.path || logPath;
                        logWriteWarning = logState.warning || logWriteWarning;
                        logWriteError = logState.error || logWriteError;
                        flushReport();
                    }
                    ...
                    if (missingCount > 0){
                        ...
                        if (!shouldProcess) alert("Operation cancelled");
                    }
                    ...
                    if (shouldProcess){
                        for (var i = 0; i < items.length; i++){
                            ...
                            if (isNaN(item.width) || isNaN(item.height)){
                                ...
                                recordRow(makeStatusRow(item.file, item.qty, item.printType, item.note, item.price, item.currency, matchInfo, item.width, item.height, "BAD_WIDTH_HEIGHT"));
                                continue;
                            }
                            ...
                            if (!matchInfo.file || !matchInfo.file.exists){
                                ...
                                recordRow(makeStatusRow(item.file, item.qty, item.printType, item.note, item.price, item.currency, matchInfo, item.width, item.height, "MISSING_FILE"));
                                continue;
                            }
                            ...
                            try {
                                ...
                                recordRow(makeStatusRow(item.file, item.qty, item.printType, item.note, item.price, item.currency, matchInfo, item.width, item.height, "OPEN_FAIL"));
                                continue;
                            }

                            try {
                                if (docHasLockedContent(doc)){
                                    missing.push(item.file + " (locked content)");
                                    recordRow(makeStatusRow(item.file, item.qty, item.printType, item.note, item.price, item.currency, matchInfo, item.width, item.height, "LOCKED_CONTENT"));
                                    try { doc.close(SaveOptions.DONOTSAVECHANGES); } catch (eCloseLocked) {}
                                    continue;
                                }
                                ...
                                if (runWeMustAction){
                                    try {
                                        ...
                                        recordRow(makeStatusRow(item.file, item.qty, item.printType, item.note, item.price, item.currency, matchInfo, item.width, item.height, "ACTION_FAIL"));
                                ...
                                app.executeMenuCommand("selectall");
                                var b0 = getSelectionBounds(doc);
                                if (!b0){
                                    missing.push(item.file + " (no selectable artwork)");
                                    recordRow(makeStatusRow(item.file, item.qty, item.printType, item.note, item.price, item.currency, matchInfo, item.width, item.height, "NO_ARTWORK"));
                                ...
                                if (cur.w <= 0 || cur.h <= 0){
                                    ...
                                    recordRow(makeStatusRow(item.file, item.qty, item.printType, item.note, item.price, item.currency, matchInfo, item.width, item.height, "BAD_BOUNDS"));
                                ...
                                if (resizeMode === "respectWidth") scaleSelection(doc, sx, sx);
                                else if (resizeMode === "respectHeight") scaleSelection(doc, sy, sy);
                                else scaleSelection(doc, sx, sy);

                                if (!fitArtboardToArtwork(doc, ARTBOARD_PADDING_PT)){
                                    missing.push(item.file + " (cannot fit artboard)");
                                    recordRow(makeStatusRow(item.file, item.qty, item.printType, item.note, item.price, item.currency, matchInfo, item.width, item.height, "FIT_ARTBOARD_FAIL"));
                                    try { doc.close(SaveOptions.DONOTSAVECHANGES); } catch (eCloseFit) {}
                                    continue;
                                }

                                app.executeMenuCommand("selectall");
                                var bOut = getSelectionBounds(doc);
                                ...
                                processed++;
                                recordRow(makeMeasuredRow(item.file, item.qty, item.printType, item.note, item.price, item.currency, matchInfo, resizeMode, item.width, item.height, outW, outH, getRelativePath(outputFile, exportFolder), outputFile.fsName));
                            } catch(eProc){
                                missing.push(item.file + " (process/export error)");
                                recordRow(makeStatusRow(item.file, item.qty, item.printType, item.note, item.price, item.currency, matchInfo, item.width, item.height, "PROCESS_ERROR"));
                            }
                        ...
                        flushReport();
                        appendManagedLog(logState, buildLogSummary(reportMeta, reportRows));
                        logPath = logState.path || logPath;
                        logWriteWarning = logState.warning || logWriteWarning;
                        logWriteError = logState.error || logWriteError;
                        ...
                        if (generateProof){
                            ...
                            proofWriteError = proofResult.ok ? "" : proofResult.error;
                            proofWriteWarning = proofResult.warning || proofWriteWarning;
                            proofPath = proofResult.path;
                        }

                        if (generatePricing){
                            var pricingResult = writePricingAuditHtml(exportFolder, reportMeta, reportRows, pricingPath);
                            pricingWriteError = pricingResult.ok ? "" : pricingResult.error;
                            pricingWriteWarning = pricingResult.warning || pricingWriteWarning;
